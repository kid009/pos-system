...
- **Auth** — custom session-based (not Breeze/Fortify). `LoginService` handles authentication with rate limiting. Roles are `admin`/`staff` stored as a `role` column on User (no Spatie).
...
- Views organized by domain: `resources/views/{admin,pos,auth,layouts,components}/`
