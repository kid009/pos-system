...
<div class="pt-3 pb-2 mb-3">
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center border-bottom pb-2 mb-4">
        <h1 class="h2 mb-0">แก้ไขช่องทางการขาย: <span class="text-primary">{{ $salesChannel->name }}</span></h1>
        ...
        <div class="col-lg-6 col-xl-5">
            <div class="card shadow-sm border-0">
                <div class="card-body p-4">
                    <form action="{{ route('sales-channels.update', $salesChannel->id) }}" method="POST">
                        @csrf
                        @method('PUT')

                        @include('master-data.sales-channel.partials._form', ['salesChannel' => $salesChannel])

                        <hr class="my-4">

                        <div class="d-flex justify-content-between align-items-center">
                            <small class="text-muted">อัปเดต: {{ $salesChannel->updated_at->format('d/m/Y H:i') }}</small>
