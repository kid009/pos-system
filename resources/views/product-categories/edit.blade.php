<x-app-layout>
    <x-slot:title>Edit Product Category</x-slot:title>

    <div class="max-w-2xl mx-auto space-y-6">

        <div class="flex items-center justify-between pb-2 border-b border-slate-200">
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-slate-900">Edit Product Category</h1>
                <p class="text-sm text-slate-500 mt-0.5">
                    Modifying category: <span class="font-semibold text-slate-800">{{ $productCategory->name }}</span>
                </p>
            </div>
            <div>
        ...
        <div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div class="px-6 py-4 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
                <h2 class="text-xs font-bold uppercase tracking-wider text-slate-700">Category Information</h2>
                <span class="text-[11px] font-mono text-slate-400">ID: #{{ $productCategory->id }}</span>
            </div>
            ...
            <form action="{{ route('product-categories.update', $productCategory) }}"
                ...
                @include('product-categories.partials.form', ['category' => $productCategory])
